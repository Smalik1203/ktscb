-- =====================================================
-- Fix ambiguous school_code references in tests RPCs
-- =====================================================
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE ci.id = (test_payload->>'class_instance_id')::uuid
      AND ci.school_code = v_school_code
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1
    FROM subjects s
    WHERE s.id = (test_payload->>'subject_id')::uuid
